...
public class Node<T>
{
    ...
    {
        int left = (LeftChild == null) ? 0 : LeftChild.BalanceRecursion(0) + 1;
        int right = (RightChild == null) ? 0 : RightChild.BalanceRecursion(0) + 1;
        return right - left;
    }
    private int BalanceRecursion(int level)
    {
        int left = (LeftChild == null) ? level : LeftChild.BalanceRecursion(level + 1);
